...
The plot illustrates the proportion of students who reported feeling more motivated to study as a result of using AI tools, segmented by how many hours per week they use them.

- Students who used AI tools for **3–5 hours per week** showed the **highest motivation rate** (61%).
- Lower motivation was reported by students using AI tools **less than 2 hours (48%)** or **6–10 hours (50%)**.
- The group using AI tools for **10+ hours** reported 56% motivation.
- One group with `NA` in `ai_hours` showed 100% motivation, which suggests incomplete data and should be excluded from final inference.

These findings suggest a **moderate positive relationship** between AI tool usage and perceived motivation, though the relationship is **not linear**. It appears that **moderate usage** may be more motivating than very low or very high usage.
...
- The **average age** of participants is **25.8 years**, with a range from **4 to 45**. The distribution appears fairly symmetric (skew = 0.30), though the extreme minimum value of 4 years likely reflects a data entry error or outlier.
...
- The coefficient for `ai_hours` was **0.0092**, with a *p*-value of **0.797**, indicating that the relationship is not statistically significant.
- The model suggests that each additional hour of AI tool use is associated with only a **0.9% increase in the log-odds of reporting increased motivation**, which is negligible and not reliable based on this data.
...
- The coefficient for `ai_hours` was **-0.0508** with a *p*-value of **0.22**, indicating **no statistically significant relationship**.
